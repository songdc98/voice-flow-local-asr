...
def build_prompt(transcript: str, config: dict[str, Any], mode: str) -> str:
    ...
    if mode == "polish_zh":
        ...
        task = "把语音识别稿忠实转换成英文 coding 输入。目标是尽量复原说话者的大部分原始意图、细节、顺序和语气，而不是写摘要。只做轻度润色：去掉语气词、无意义停顿、明显重复和明显 ASR 错词；保留约束、原因、担忧、例子、优先级、页数限制、实验细节、决策授权和中英文夹杂中的技术术语。"
    else:
        task = "把语音识别稿忠实翻译成自然英文聊天消息。目标是保留大部分原始语义细节和说话顺序，只做轻度润色，不要高度概括。不要保留中文短语，专有名词除外。"
...
- 英文输出也必须低压缩：除非原文很短，否则不要压成两三句摘要；应保留原文大部分信息密度，可以按原顺序整理成几段自然英文。
- 可以小修表达，让英文更清楚、更像自然的人类输入，但不要改写成你自己的计划，不要替用户重新组织成全新的简报。
